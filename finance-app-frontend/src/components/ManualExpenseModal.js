...
const CATEGORIES = [
    'Market', 'Gıda', 'Ulaşım', 'Barınma', 'Faturalar',
    'Eğlence', 'Sağlık', 'Eğitim', 'Giyim', 'Diğer'
];
...
const ManualExpenseModal = ({ isOpen, onClose, onSave }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData.merchant_name || !formData.total_amount) {
            toast.show.warning('Lütfen satıcı adı ve tutar giriniz.');
            setLoading(false);
            return;
        }
        ...
        try {
            ...
            toast.show.success('Harcama başarıyla eklendi.');
            ...
            console.error(error);
            toast.show.error('Ekleme başarısız oldu.');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-md border border-slate-200 dark:border-slate-800 overflow-hidden animate-in fade-in zoom-in duration-200">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Satıcı / Yer</label>
                        <input
                            type="text"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                className="w-full px-4 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 outline-none focus:ring-2 focus:ring-indigo-500/20 text-sm font-medium text-slate-600"
                            />
                        </div>
                    </div>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            {CATEGORIES.map(cat => (
                                <button
                                    ...
                                    className={`px-2 py-2 text-[10px] font-bold rounded-lg border transition-all truncate ${formData.category_name === cat
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            className="w-full px-4 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 outline-none focus:ring-2 focus:ring-indigo-500/20 text-sm font-medium resize-none"
                        />
                    </div>
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-3 rounded-xl shadow-lg shadow-slate-200 dark:shadow-none transition-all flex justify-center items-center gap-2 disabled:opacity-70 mt-2"
                    >
